...
const UPLOAD_DIR = process.env.UPLOAD_DIR || './incoming';
const DOWNLOAD_DIR = process.env.DOWNLOAD_DIR || './uploads';
...
export class FileStorageService {
  ...
  async getFilesBySession(sessionId: string): Promise<IFileRecord[]> {
    return Array.from(this.fileRecords.values()).filter((record) => record.sessionId === sessionId);
  }
  ...
  async getFileStream(
    fileId: string
  ): Promise<{ contentType: string; fileName: string; stream: NodeJS.ReadableStream } | null> {
    const fileRecord = this.fileRecords.get(fileId);
